...
public:
    ...
    queue<string> localTempVars;
    ScopeTable(int n, string scopeName){
        totalBuckets = n;
        name = scopeName;
    ...
    bool Insert(string name, string type, string dataType, Attribute attribute){
        ...
        if(Lookup(name) == NULL){
            ...
            SymbolInfo* symbolInfo = new SymbolInfo(name, type, dataType, attribute);
    ...
    bool Insert(string name, string type, string dataType, Attribute attribute, string asmSymbol, string code, int arrSize){

        if(Lookup(name) == NULL){

            SymbolInfo* symbolInfo = new SymbolInfo(name, type, dataType, attribute, asmSymbol, code);

            if(arrSize > 0){
                symbolInfo->setArraySize(arrSize);
            }
