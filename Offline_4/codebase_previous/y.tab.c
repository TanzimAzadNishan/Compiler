...
void insertParameters(SymbolInfo* &funcSymbol){
	...
	for(int i = 0; i < parametersInfo.size(); i++){
		...
		paramSymbol->setAsmSymbol("[bp+" + to_string((parametersInfo.size() - (1+i))*2 + 12) + "]");
...
{
       0,   155,   155,   185,   195,   205,   212,   219,   228,   248,
     262,   269,   279,   278,   340,   339,   416,   435,   448,   463,
     477,   489,   497,   509,   517,   527,   538,   547,   556,   563,
     570,   580,   599,   619,   637,   661,   667,   673,   678,   685,
     697,   716,   722,   728,   728,   743,   763,   779,   799,   817,
     837,   846,   853,   862,   867,   875,   880,   888,   910,   942,
     950,   980,   989,  1031,  1040,  1087,  1097,  1128,  1138,  1198,
    1216,  1232,  1243,  1252,  1323,  1331,  1338,  1345,  1358,  1371,
    1377,  1384,  1390,  1403
};
#endif
...
#line 156 "1705074.y"
		...
		if(error_count == 0){
			...
			asmFile << appendStackDataSegment();
			asmFile << ".CODE\n\n";	
   			if(yyvsp[0]->getCode().find("println") != std::string::npos){
   				asmFile << getAsmOfPrintln();
   			}
			
			asmFile << yyvsp[0]->getCode();
			asmFile << "\nEND MAIN\n";
		}
...
#line 279 "1705074.y"
			  		...
			  		if(isConsistent){
						...
						/*if(funcSymbol->getDataType() != "void"){
							string tempVar = getTempVar("", "", false, false);
							funcSymbol->setFuncAsmSymbol(tempVar);
						}*/
				  		if(!isRedeclared){
...
#line 328 "1705074.y"
			...
			string code = getAsmOfParameterizeFunction(yyvsp[-5], yyvsp[0]);
...
#line 340 "1705074.y"
				...
				if(currentFuncSymbol->getDataType() != "void"){
					string tempVar = getTempVar("", "", false, false);
					currentFuncSymbol->setFuncAsmSymbol(tempVar);
				}*/		  			 	
		  }
#line 1880 "y.tab.c"
    break;
...
#line 375 "1705074.y"
			...
			string code = "";
			string returnLabel = "ret_" + yyvsp[-4]->getName();
...
#line 990 "1705074.y"
		 	...
		 	string tempVar = getTempVar(yyvsp[-2]->getAsmSymbol(), yyvsp[0]->getAsmSymbol(), yyvsp[-2]->isNumber, yyvsp[0]->isNumber);
		 	string label1 = newLabel();
			string label2 = newLabel();
...
#line 1139 "1705074.y"
			...
			yyval = new SymbolInfo(newName, "term", dataType, yyvsp[-2]->getAttribute(), tempVar, code);
...
#line 1199 "1705074.y"
		 	...
		 	string newName = yyvsp[-1]->getName() + yyvsp[0]->getName();
			string code = "";
			string tempVar = getTempVar(yyvsp[0]->getAsmSymbol(), "", yyvsp[0]->isNumber, -1);
			...
			yyval = new SymbolInfo(newName, "unary_expression", yyvsp[0]->getDataType(), yyvsp[0]->getAttribute(), tempVar, code);
			yyval->isNumber = yyvsp[0]->isNumber;
...
#line 1346 "1705074.y"
        {
	 	string newName = yyvsp[-1]->getName() + "++";
	 	string tempVar = getTempVar(yyvsp[-1]->getAsmSymbol(), "", yyvsp[-1]->isNumber, -1);
